...
app.post('/api/v1/articles/:slug/comments',{preHandler:requireUser,config:{rateLimit:{max:5,timeWindow:'1 minute'}}},async(request:any,reply)=>{const body=cleanText(request.body?.body);if(!body||body.length>2000)return reply.code(400).send({error:'invalid_comment'});const parentId=request.body?.parentId||null;if(parentId){const parent=await query(`SELECT 1 FROM comments WHERE id=$1 AND article_slug=$2 AND status='visible'`,[parentId,request.params.slug]);if(!parent.rowCount)return reply.code(400).send({error:'invalid_parent_comment'})}const r=await query<any>(`INSERT INTO comments(article_slug,user_id,parent_id,body) VALUES($1,$2,$3,$4) RETURNING id`,[request.params.slug,request.user.id,parentId,body]);return reply.code(201).send({id:String(r.rows[0].id)})})
...
app.delete('/api/v1/admin/users/:id/ban',{preHandler:requireAdmin},async(request:any,reply)=>{const r=await query<any>(`UPDATE users SET banned_at=NULL,updated_at=now() WHERE id=$1 AND banned_at IS NOT NULL RETURNING id`,[request.params.id]);if(!r.rowCount)return reply.code(409).send({error:'user_not_banned'});await query(`INSERT INTO moderation_logs(admin_id,action,target_type,target_id) VALUES($1,'unban','user',$2)`,[request.user.id,request.params.id]);return{ok:true}})
