  ...
  <div class="admin-page page-enter">
    <div class="container">
      ...
      <template v-else-if="currentUser?.role === 'admin'">
        ...
        <section><h2>评论管理</h2><div class="table-wrap"><table><thead><tr><th>用户</th><th>文章</th><th>内容</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="item in comments" :key="item.id"><td>{{ item.author.login }}</td><td>{{ item.articleSlug }}</td><td class="comment-body">{{ item.body }}</td><td>{{ statusLabels[item.status] || item.status }}</td><td><button @click="moderate(item,'visible')">显示</button><button @click="moderate(item,'hidden')">隐藏</button><button v-if="item.status !== 'deleted'" class="danger" @click="moderate(item,'deleted')">删除</button><button v-else class="danger" @click="permanentlyDelete(item)">彻底删除</button></td></tr></tbody></table></div></section>
        <section><h2>用户管理</h2><div class="users"><article v-for="user in users" :key="user.id"><img :src="user.avatarUrl" alt=""><div><strong>{{ user.login }}</strong><small>{{ user.role }} · {{ user.bannedAt ? '已封禁' : '正常' }}</small></div><button v-if="!user.bannedAt && user.role !== 'admin'" class="danger" @click="ban(user)">封禁</button><button v-else-if="user.bannedAt" class="unban" @click="unban(user)">解封</button></article></div></section>
      </template>
...
const statusLabels={visible:'显示',hidden:'隐藏',deleted:'已删除'}
