      ...
      <aside class="meta-panel">
        ...
        <section class="image-section">
          ...
          <div class="image-list">
            <div v-for="image in bodyImages" :key="image.name" class="image-item">
              <button class="image-insert" type="button" title="插入正文" @click="insertImage(image)">
                <img :src="image.url" :alt="image.name" />
              </button>
              <div class="image-info">
                <span :title="image.name">{{ image.name }}</span>
                <div class="image-actions">
                  <button class="image-rename" type="button" :aria-label="`重命名图片：${image.name}`" @click="renameBodyImage(image)">重命名</button>
                  <button class="image-remove" type="button" :aria-label="`删除图片：${image.name}`" title="删除图片并移除正文引用" @click="removeImage(image)">删除</button>
                </div>
              </div>
            </div>
          </div>
